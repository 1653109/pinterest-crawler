...
def dowload_single_image(myinput, mydir):
...
def download(myinput, mydir = "./"):
    ...
    if isinstance(myinput, list):
        ...
        for i in myinput:
            # download(i, mydir)
            step += 1
            printProgressBar(step, l, prefix = 'Progress:', suffix = 'Complete', length = 50)
            dowload_single_image(i, mydir)
    else:
        pass
...
class Pinterest_Helper(object):
    ...
    def __init__(self, login, pw, browser = None):
        ...
        loginBtn = self.browser.find_element_by_css_selector('[data-test-id="simple-login-button"]')
        loginBtn.click();
